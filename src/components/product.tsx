    ...
    <TouchableOpacity ref={ref} className="w-full flex-row items-center pb-4" {...rest}>
      ...
      <View className="flex-1 ml-3">
        <View className="flex-row items-center gap-2">
          ...
          {data.quantity && (
            <Text className="text-lime-300 font-subtitle text-sm">x {data.quantity}</Text>
          )}
        </View>

        <Text className="text-slate-400 font-body text-xs leading-5 mt-0.5">{data.description}</Text>
      </View>
    </TouchableOpacity>
  )
})